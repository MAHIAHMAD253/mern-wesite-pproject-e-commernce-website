...
const Add = ({token}) => {
  ...
  const onSubmitHandler = async  (e) =>{
    ...
    try {
      ...
      const resp = await axios.post(backendUrl + '/api/product/add', formData,{headers:{token}})
...
{/* ------------------------------------Input Section -------------------------- */}
      ...
      <div className="flex flex-col sm:flex-row gap-3 w-full sm:gap-8">
        <div>
          ...
          <select onChange={(e) => setCategory(e.target.value)} className="w-full py-3 px-2">
        ...
        <div>
          <p className="mb-2">SubCategory Products</p>
          <select onChange={(e) => setSubCategory(e.target.value)}  className="w-full py-3 px-2">
      ...
      <div>
        <p className="mb-3">Products Size</p>
      ...
      <button className="w-28 bg-black text-white py-3 px-5 " type="submit">
        Add
      </button>
    </form>
  );
